...
public class GetListProductCommandImpl implements GetListProductCommand {
    ...
    public GetListProductCommandResponse doCommand(GetListProductCommandRequest commandRequest) {
        log.info(
                "Executing GetListProductCommand with search: {}, category: {}, minPrice: {}, maxPrice: {}, brand: {}, page: {}, size: {}",
                commandRequest.getSearch(), commandRequest.getCategory(), commandRequest.getMinPrice(),
                commandRequest.getMaxPrice(), commandRequest.getBrand(), commandRequest.getPage(),
                commandRequest.getSize());
        ...
        GetListProductCommandResponse cache = null;
        try {
            cache = (GetListProductCommandResponse) cacheService.get(key);
            if (cache != null) {
                return cache;
            }
        } catch (Exception e) {
            log.warn("Failed to retrieve from cache for key: {}. Error: {}", key, e.getMessage());
        }

        log.info("Fetching products from database with filters");
        ...
        try {
            ...
            log.warn("Failed to cache product list for key: {}. Error: {}", key, e.getMessage());
        }
